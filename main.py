...
# SITE_PASSWORD - The password to access the site (required for protection)
# SESSION_SECRET - Secret key for session signing (auto-generated if not set)

SITE_PASSWORD = os.environ.get("SITE_PASSWORD", "")  # Empty = no protection
...
def hash_password(password: str) -> str:
    ...
    return hashlib.sha256((password + SESSION_SECRET[:16]).encode()).hexdigest()

def create_session() -> str:
    """Create a new session token."""
    token = secrets.token_urlsafe(32)
    active_sessions[token] = datetime.now() + timedelta(hours=SESSION_DURATION_HOURS)
    return token
...
class AuthMiddleware(BaseHTTPMiddleware):
    """Middleware to protect routes with password authentication."""
    
    # Routes that don't require authentication
    PUBLIC_ROUTES = {"/login", "/api/login", "/api/health", "/favicon.ico"}
...
# Add auth middleware (only if password is set)
if SITE_PASSWORD:
    app.add_middleware(AuthMiddleware)
    print(f"[Auth] Password protection ENABLED")
else:
    print(f"[Auth] Password protection DISABLED (set SITE_PASSWORD to enable)")
...
class ExportSettings(BaseModel):
    ...
    # Audio enhancement (lightweight, works on free tier)
    enhance_audio: bool = False
    noise_reduction: float = Field(default=0.7, ge=0.0, le=1.0)
...
async def export_final_video(
    ...
    try:
        ...
        if settings.enhance_audio:
            try:
                from audio_processor import enhance_audio_basic
                print(f"[Export] Applying audio enhancement...")
                
                # Enhance the exported video
                enhanced_path = output_dir / f"enhanced_{output_filename}"
                audio_stats = enhance_audio_basic(
                    output_path,
                    enhanced_path,
...
async def export_audio_from_video(
    ...
    job = db.query(Job).filter(Job.id == job_id).first()
    output_dir = Path(job.output_dir) if job else app_config.outputs_dir / job_id
...
async def import_audio_to_video(
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_filename = f"voice_swapped_{timestamp}.mp4"
        output_path = output_dir / output_filename
...
async def list_job_outputs(job_id: str, db: Session = Depends(get_db_session)):
...
async def voice_swap_video_endpoint(
    ...
    try:
        ...
        # Create output path
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_filename = f"voice_cloned_{timestamp}.mp4"
        output_path = output_dir / output_filename

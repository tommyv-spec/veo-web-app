...
class APIKeysConfig:
    ...
    def _load_blocked_keys(self):
        """Load blocked keys from disk and clean up expired ones"""
        try:
            if self._blocked_keys_file and self._blocked_keys_file.exists():
                ...
                now = datetime.now()
                expired_count = 0
                for key_index, block_time in list(loaded_keys.items()):
                    unblock_time = block_time + timedelta(hours=self.block_duration_hours)
                    if now >= unblock_time:
                        del loaded_keys[key_index]
                        expired_count += 1
                
                self.blocked_keys = loaded_keys
                
                if expired_count > 0:
                    print(f"[APIKeys] ✅ Auto-unblocked {expired_count} expired keys", flush=True)
                    self._save_blocked_keys()  # Save cleaned-up state
                
                if len(self.blocked_keys) > 0:
                    print(f"[APIKeys] {len(self.blocked_keys)} keys still blocked (will expire in <{self.block_duration_hours}h)", flush=True)
                else:
                    print(f"[APIKeys] All keys available", flush=True)

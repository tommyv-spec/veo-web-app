...
def export_final_video(
    clip_info: List[dict],
    output_path: Path,
    frames_to_cut_start: int = 7,
    frames_to_cut_end: int = 7,
    ...
    Args:
        ...
        frames_to_cut_start: Frames to trim from start of each clip (unless skip_start_trim)
        frames_to_cut_end: Frames to trim from end of each clip
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for idx, info in enumerate(clip_info, 1):
            ...
            # Apply start trim only if not skipped
            actual_start_trim = 0 if skip_start else frames_to_cut_start
            
            logger.info(f"Clip {info.get('clip_index', idx)}: start_trim={actual_start_trim}, end_trim={frames_to_cut_end}, skip_start={skip_start}")
        ...
        concat_videos(trimmed_files, concat_output)
